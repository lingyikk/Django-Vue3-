<template>
  <AppLayout>
      ...
      <el-row class="mb-4 nav-rows">
        ...
        <div class="mt-4">
          <el-input v-model="inputSearchData" style="max-width: 600px" placeholder="用户名" class="input-with-select"
            ...
            <template #append>
              <el-button :icon="Search" @click="searchadminname" />
            </template>
          </el-input>
        </div>
      </el-row>
    ...
    <template v-slot:main>
      <!-- 表格 -->
      <el-table ref="multipleTableRef" :data="user_list?.data" style="width: 100%"
...
import { getadminAll, postadminAddInput, updataadmin, deleteadmin, searchadmin } from "@/service";
...
const table_header = [
  ...
  { property: 'username', label: "用户名", width: "auto" },
  { property: 'password', label: "密码", width: "auto" },
...
const tableSubmit = () => {
  if (isEidt.value) {
    ...
    updataadmin(oldUpdate.value.id, ruleForm)
      .then(res => {
        // 弹出框
        ElMessage({
          message: '提交成功',
...
const updataTable = (index) => {
  dialogVisible.value = true
  // 获取当前点击修改按钮的行数据
  oldUpdate.value = user_list.value.data[index]
  // 将数据添加到表单上
...
const deleteTr = (index) => {
  deleteadmin(user_list.value.data[index].id)
    .then(res => {
      // 弹出框
      ElMessage({
        message: '删除成功',
...
const searchadminname = () => {
  console.log(user_list.value);

  searchadmin(inputSearchData.value)
    .then(res => {
      // console.log(res);
      user_list.value = res
    }).catch(err => {
