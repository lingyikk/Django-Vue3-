  ...
  <AppLayout>
    <template v-slot:nav>
      <el-row class="mb-4 nav-rows">
        ...
        <div class="mt-4">
          <el-input v-model="inputSearchData" style="max-width: 600px" placeholder="酒店名" class="input-with-select"
            size="default">
            <template #prepend>
              搜索
            </template>
            <template #append>
              <el-button :icon="Search" @click="searchhotel" />
            </template>
          </el-input>
        </div>
      </el-row>
      ...
      <el-dialog v-model="dialogVisible" width="30%">
        ...
        <el-form ref="ruleFormRef" :model="ruleForm" label-width="120px" class="demo-ruleForm"
          ...
          <el-form-item label="酒店名称" prop="name">
            <el-input v-model="ruleForm.name" placeholder="酒店名称" />
          </el-form-item>
          <el-form-item label="具体地址" prop="addr">
            <el-input v-model="ruleForm.addr" placeholder="具体地址" />
          </el-form-item>
          <el-form-item label="所在城市" prop="city">
            <el-input v-model="ruleForm.city" placeholder="所在城市" />
          </el-form-item>
          <el-form-item label="价格" prop="price">
            <el-input v-model="ruleForm.price" placeholder="价格" />
          </el-form-item>
          <el-form-item label="剩余房间数" prop="stock">
            <el-input v-model="ruleForm.stock" placeholder="剩余房间数" />
          </el-form-item>
          <el-form-item label="酒店描述" prop="detail">
            <el-input v-model="ruleForm.detail" placeholder="酒店描述" />
          </el-form-item>
        </el-form>
        <template #footer>
...
import { getHotel, postHotel, updataHotel, delHotel, searchHotel  } from "@/service";
...
const table_header = [
  ...
  { property: 'name', label: "酒店名称", width: "200" },
  { property: 'city', label: "所在城市", width: "130" },
  { property: 'addr', label: "具体地址", width: "130" },
  { property: 'price', label: "价格", width: "auto" },
  { property: 'stock', label: "剩余房间数", width: "110" },
  { property: 'detail', label: "酒店描述", width: "430" },
]
...
const tableSubmit = () => {
  if (isEidt.value) {
    ...
    updataHotel(oldUpdate.value.id, ruleForm)
      .then(res => {
        // 弹出框
        ElMessage({
          message: '提交成功',
...
const updataTable = (index) => {
  ...
  ruleForm.addr = oldUpdate.value.addr
  ruleForm.city = oldUpdate.value.city
...
const searchhotel = () => {
  ...
  searchHotel(inputSearchData.value)
    .then(res => {
      list.value = res
      console.log(res);
    }).catch(err => {
