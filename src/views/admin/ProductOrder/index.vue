      ...
      <el-dialog v-model="dialogVisible" width="30%">
        ...
        <el-form ref="ruleFormRef" :model="ruleForm" label-width="120px" class="demo-ruleForm"
          ...
          <el-form-item label="产品名称" prop="description">
            <el-input v-model="ruleForm.product" placeholder="产品名称" />
          </el-form-item>
...
import { getProductOrder, postProductOrder, updataProductOrder, delProductOrder, searchProductOrder } from "@/service";
...
// 表单字段数据
const table_header = [
  { property: 'id', label: "id", width: "auto" },
  { property: 'user__username', label: "用户", width: "auto" },
  { property: 'product__name', label: "产品名称", width: "240" },
...
const tableSubmit = () => {
  if (isEidt.value) {
    ...
    updataProductOrder(oldUpdate.value.id, ruleForm)
      .then(res => {
        // 弹出框
        ElMessage({
          message: '提交成功',
...
const updataTable = (index) => {
  ...
  ruleForm.product = oldUpdate.value.product__name
...
const deleteTr = (index) => {
  delProductOrder(list.value[index].id)
    .then(res => {
      // 弹出框
      ElMessage({
        message: '删除成功',
...
const select = () => {
  ...
  searchProductOrder(inputSearchData.value)
    .then(res => {
      list.value = res
      console.log(res);
    }).catch(err => {
