    ...
    <template v-slot:main>
      ...
      <el-dialog v-model="dialogVisible" width="30%">
        ...
        <el-form ref="ruleFormRef" :model="ruleForm"  label-width="120px" class="demo-ruleForm"
          ...
          <el-form-item label="景区" prop="description">
            <el-input v-model="ruleForm.ScenicSpot" placeholder="景区" />
          </el-form-item>
          <el-form-item label="数量" prop="price">
            <el-input v-model="ruleForm.quantity" placeholder="数量" />
          </el-form-item>
          <el-form-item label="状态" prop="stock">
            <el-input v-model="ruleForm.status" placeholder="状态" />
          </el-form-item>
          <el-form-item label="总价" prop="stock">
...
import { getScenicSpotOrder, postScenicSpotOrder, updataScenicSpotOrder, delScenicSpotOrder, searchScenicSpotOrder } from "@/service";
...
const table_header = [
  ...
  { property: 'ScenicSpot__name', label: "景区", width: "auto" },
...
const tableSubmit = () => {
  if (isEidt.value) {
    ...
    updataScenicSpotOrder(oldUpdate.value.id, ruleForm)
      .then(res => {
        list.value = res.newData
...
const updataTable = (index) => {
  ...
  ruleForm.ScenicSpot = oldUpdate.value.ScenicSpot__name
...
const deleteTr = (index) => {
  delScenicSpotOrder(list.value[index].id)
    .then(res => {
      // 弹出框
      ElMessage({
        message: '删除成功',
...
const select = () => {
  ...
  searchScenicSpotOrder(inputSearchData.value)
    .then(res => {
      list.value = res
      console.log(res);
    }).catch(err => {
...
//   searchScenicSpotOrder(newValue)
//     .then(res => {
//       list.data = res;
//     })
//     .catch(err => {
